	...
	var datasetGraph = "<http://data-gov.ie/graph/sii>";
	...
	$(document).ready(function() {
		...
		if (locParameterURI=="" && window.locationName == undefined) { //Access to the app from the first page - no parameteres
			...
			locParameterURI = locParameterURI.replace(/-/g," ");
			switchLayers('results');
			buildFirstPageSkeleton();
			buildMap();
			validateLocation(locParameterURI);
	...
	function buildQueryForCheckingLocation(locParameterURI) {
		//special case of County Dún Laoghaire-Rathdown
		if (locParameterURI.indexOf("Laoghaire Rathdown")!=-1)
			locParameterURI = "Laoghaire-Rathdown";
		var sparql =
	...
	function displayRegions(cities,counties) {
		...
		//var html = "<div id='list4' style='float:left;'><ul><li><h2>County<\/h2><ul>";
		...
		//html += "<\/ul><\/li><\/ul></div><div id='list4' style='float:left;'><ul><li ><h2>City<\/h2><ul>";
	...
	function loadDBpediaData(uri,name) {
		...
		if (name.indexOf("County")!=-1) {
			...
			rows = queryDBpediaCityResults.results.bindings;
		var region;
		$("#dbpedia-content-holder").html("");
		for(i in rows) {
			var row = rows[i];
			var lname = row.label.value;
			console.log(row.label.value + ' = ' + name);
			if (lname==name || (lname.indexOf(name)!=-1 || name.indexOf(lname)!= -1) ) {
				region = new Object();
	...
	function displayDBpediaData(region) {
		...
		$("#dbpedia-content-holder").html("<a href=\""+region.name+"\" target='_blank'>" + locationName + " in wikipedia </a><br>" + "<img id=\"myImage\"src=\"" + img + "\"><br><a href=\""+region.web+"\" target=\"_blank\">" + region.web +"<\/a>"); 
		
		$("#myImage").error(function(){
			$("#myImage").attr("src","img/no-image.gif");
		});
	...
	function displayCensusData(region) {
		if (typeof(region.population)!='undefined')
	...
	function setVisible(parent,visible,level) {
		...
		for (var i=0;i<idsValues.length;i++) {
			...
			if (div.indexOf(parent)!=-1 && div!=parent && itemLevel == level ) {
				//console.log('-------- <'+div+'> <' + parent + '>' + level + ' itemlevel ' + itemLevel );
				if (visible=='true') {
